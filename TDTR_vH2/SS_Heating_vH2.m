function dTss = SS_Heating_vH2(matparams,r_pump,r_probe,absC,A_tot,intscheme,nnodes)
%SS_Heating_vH2 - Calculates the steady state heating of an anisotropic 
%multilayer stack. Can handle bidirectional heat flow.
%
% Syntax:  SS_Heating_vH2(matparams,r_pump,r_probe,absC,A_tot,intscheme,nnodes)
%
% Inputs:
...
if intscheme == 2 % Simpson integration
    % RW preference
    kmax = 2/sqrt(r_pump^2+r_probe^2);
else % go with JPF / prior factors.
    kmax = 1.5/sqrt(r_pump^2+r_probe^2);
end
...
if BI
    ...
    switch intscheme
        case 0 % Legendre-Gauss
            ...
            Id = TDTR_TEMP_vH2(kvect,f,LCTEd,r_pump,r_probe,A_abs);
            dTd = weights'*Id;
        case 1 % rombint
            dTu=rombint_VV3(@(kvect) TDTR_TEMP_vH2(kvect,f,LCTEu,r_pump,r_probe,A_abs),kmin,kmax,1);
            dTd=rombint_VV3(@(kvect) TDTR_TEMP_vH2(kvect,f,LCTEd,r_pump,r_probe,A_abs),kmin,kmax,1);
    ...
    switch intscheme
        case 0 % Legendre-Gauss
            I = TDTR_TEMP_vH2(kvect,f,LCTE,r_pump,r_probe,A_abs);
            dTss = weights'*I;
        case 1 % rombint
            dTss=rombint_VV3(@(kvect) TDTR_TEMP_vH2(kvect,f,LCTE,r_pump,r_probe,A_abs),kmin,kmax,1);
        case 2 % Simpson
            kvect = linspace(kmin,kmax,nnodes)';
            I = TDTR_TEMP_vH2(kvect,f,LCTE,r_pump,r_probe,A_abs);
