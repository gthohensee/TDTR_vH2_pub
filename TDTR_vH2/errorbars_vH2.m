function [kErr_perc, kErr_abs, ErrSummary_perc, ErrSummary_abs] = ...
    errorbars_vH2(XguessIJ,datparams,sysparams,calparams,matparams,Tparams,options)
%errorbars_vH2 - calculates error bars for the Xijc fit parameters
%based on uncertainties in other thermal/system parameters. Sequentially
%calls TDTR_FIT_vH2.m to calculate how much the fitted Xsol changes
...
%specified by errorbar_conditions_vH2.m script.
%
% Inputs:
...
%
% Other m-files required: TDTR_FIT_vH2.m, TDTR_REFL_vH2.m,
...
%------------- BEGIN CODE --------------
%% unpack inputs
INITIALIZE_CELLPARAMS_vH2; % unpacks/cleans the five cellparams

%% Initialize variables and uncertainties
Xij = XguessIJ(:,2:3);
Xguess = XguessIJ(:,1);
errorbar_conditions_vH2; % default uncertainties
...
Xsol=fminsearch(@(X) TDTR_FIT_vH2(X,Xij,datparams,sysparams,calparams,...
...
for i = 1:4
    for j=1:nl
        ...
        Xsoltemp=fminsearch(@(X) TDTR_FIT_vH2(X,Xij,datparams,sysparams,calparams,...
                              matparams,Tparams),Xguess,options);
...
if phase_consider==1
    ...
    if vinfit, datparams{2} = Vin_phaseshifted;
    else datparams{2} = ratio_phaseshifted; end
    ...
    if vinfit, datparams{2} = Vin_data; % return to reference value
    else datparams{2} = ratio_data; end
end
...
ErrSummary_abs = vertcat(ErrSummary_abs,...
                         ErrSummary_abs_sys);
