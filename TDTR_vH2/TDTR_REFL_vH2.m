function [deltaR,ratio]=TDTR_REFL_vH2(tdelay,matparams,sysparams,A_pump,...
...
%
% Syntax:  [deltaR,ratio]=TDTR_REFL_vH2(tdelay,matparams,sysparams,A_pump,...
%                                     intscheme,nnodes,Fortran)
%
...
%
% Other m-files required: TDTR_TEMP_vH2.m
% Subfunctions: none
...
if nargin < 3, error('Insufficient parameters for TDTR_REFL_vH2.'); end
...
if ~Fortran || ~BI % BI = 0 overrides Fortran = 1    
    ...
    kmin = 0;
    kmax=cap2/sqrt(max(r_pump)^2+r_probe^2);
    ...
    if BI
        ...
        switch intscheme
            case 0 % Legendre-Gauss
                ...
                I1d = TDTR_TEMP_vH2(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump);
                I2d = TDTR_TEMP_vH2(kvect,mvect/tau_rep-f,LCTEd,r_pump,r_probe,A_pump);
            ...
            case 1 % rombint
                ...
                dT1d=rombint_VV3(@(kvect) TDTR_TEMP_vH2(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                dT2d=rombint_VV3(@(kvect) TDTR_TEMP_vH2(kvect,mvect/tau_rep+f,LCTEd,r_pump,r_probe,A_pump),0,kmax,length(mvect));
        ...
        switch intscheme
            case 0 % Legendre-Gauss
                ...
                I1 = TDTR_TEMP_vH2(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump);
                I2 = TDTR_TEMP_vH2(kvect,mvect/tau_rep-f,LCTE,r_pump,r_probe,A_pump);
            ...
            case 1 % rombint
                dT1=rombint_VV3(@(kvect) TDTR_TEMP_vH2(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump),0,kmax,length(mvect));
                dT2=rombint_VV3(@(kvect) TDTR_TEMP_vH2(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump),0,kmax,length(mvect));
            case 2 % Simpson
                kvect = linspace(0,kmax,nnodes)';
                I1 = TDTR_TEMP_vH2(kvect,mvect/tau_rep+f,LCTE,r_pump,r_probe,A_pump);
                I2 = TDTR_TEMP_vH2(kvect,mvect/tau_rep-f,LCTE,r_pump,r_probe,A_pump);
