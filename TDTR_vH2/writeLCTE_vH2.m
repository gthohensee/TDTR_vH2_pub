function [LCTE, T_LCTE] = writeLCTE_vH2(stack, thickness,rho_Al,T0,Xijc,refdir,P)
...
% 
% Syntax:  [LCTE, T_LCTE] = writeLCTE_vH2(stack, thickness,rho_Al,T0,Xijc,refdir,P)
%
% Inputs:
...
%                Very common in TDTR experiments.
...
%
% Example: 
%    LCTE = writeLCT({'*','Al','Al/SiO2','SiO2','Si'}, [1 80 1 104 500e3], 3.9e-8, 120, Xij, -1)
%    LCTE = writeLCT({'diamond','d/Al','*','Al','Soil_1cSt'}, [100e3 1 1 120 30e3], -1, -1, 0, 11)
%
% Other m-files required: P-dependence and T-dependence functions.
% Subfunctions: none
% MAT-files required: none
%
% See also: writeLCTEg_vH2.m
...
%                            10-Sept-2014 - cleaned up entries, removed
%                                           P-dependence for diamond.
%------------- BEGIN CODE --------------
L0 = 2.445e-8; % constant, Wiedermann-Franz law.
RT = 295; % room temperature (K) default

%% check inputs
if nargin < 3, rho_Al = -1; LAl = 150; % sputter 3 gives 150-160 W/m-K.
else
    if rho_Al < 0 % "ignore rho_Al" default -- gives 192 W/m-K L(Al).
        rho_Al = 3.90e-8;
    end
    LAl = L0*RT/rho_Al; % default: 295 K aluminum conductivity given rho_Al
    % The thermal conductivity of Al depends on the sputtering chamber used.
    % Sputter 1 usually gives 180+ W/m-K.    
end
if nargin < 4, T0 = -1; end % default no temperature dependence
if nargin < 5, Xijc = 0; % default no fit parameter
else
    if length(Xijc(1,:)) == 2 % user gave Xij instead of Xijc
        Xijc = horzcat(Xijc,ones(length(Xijc(:,1)),1)); % assume all parameters are in LCTE.
    end
end
if nargin < 6 || ~exist('refdir','var')
    refdir = '/Users/gregoryhohensee/Documents/MATLAB/projectbacon/exportbacon/TTM/refdir'; 
end

if nargin < 7
    if isnumeric(refdir) % stupid user includes P but forgets to include refdir
        P = refdir; 
    else
        P = -1;
        fprintf('DEFAULTING TO ATMOSPHERIC PRESSURE\n');
    end
end
            ...
            case '*' % absorption layer cases
                ...
                switch ss
                    ...
                    case 'msi' % metallic silicon, P > 13 GPa or so.
                        ...
                        C(ii+1) = 1.64e6;
                        if P ~= -1
                            [PSi,~,~,~,~,CSi] = empirical_PXK_Si_v2(P); % uses new C_Debye(Si).
                            [~,index] = min(abs(PSi - P));
                            C(ii+1) = CSi(index)*1e6;
                        end
                    otherwise
                        fprintf('ERROR:writeLCT; absorption layer for what material?\n')
...
%% OTHER MATERIALS % % % % % % % % % % % % % % % % % % % % % % % % % % % %
            case 'msi' % metallic silicon at high pressure
                lambda(ii) = 200;
                C(ii) = 1.64e6;
                if P ~= -1
            ...
            case 'diamondanvil'
                lambda(ii) = 500;
                C(ii) = 1.81e6;
                % Diamond heat capacity vs. pressure is unknown,
                % probably doesn't scale with Debye model, if it's anything like MgO.
%                 if P ~= -1
...
%                 end
            case 'diamond'
                lambda(ii) = 2400;
                C(ii) = 1.81e6;
            case '2a' % Type 2A synthetic diamond
                lambda(ii) = 2400;
                C(ii) = 1.81e6;
            case '1a' % Type 1A natural diamond
                lambda(ii) = 700;
                C(ii) = 1.81e6;
            ...
            case 'soil_1cst'
                ...
                if P ~= -1
                    ...
                    C(ii) = (1.5*rhoS*1e-3)*1e6; % J/m^3-K
            ...
            case 'soil_30000cst'
                ...
                lambda(ii) = 0.15;
                if P ~= -1
                % from Wen-Pin Hsieh PRB 2011 paper. All other parameters effectively
                % constant or irrelevant with pressure, in silicone/AuPd/D case.
