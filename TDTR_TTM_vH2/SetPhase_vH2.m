function [delphase,phase,fitparam] = SetPhase_vH2(data,t0,filename,pathname,t_window)
%SetPhase_vH2 - Adjust the phase defining V(in) and V(out) in DATA matrix.
...
%do this efficiently.
%
% Syntax:  [delphase,phase,fitparam] = SetPhase_vH2(data,t0,filename,pathname,t_window)
%
% Inputs:
...
%    t_window - optional parameter for set_t0.m; default is [-10 10] ps.
%               Specifies the domain for the figure for picking t0.
%
% Outputs:
...
% Example:
%    [delphase, phase] = SetPhase_vH2(data,-0.3,filename,pwd, [-20 80])
%
...
[~,ishort] = min(abs(tdelay_data_B - tdelay_short));
...
while ~done
    ...
    switch option
        ...
        case 'f' % manually adjust the linear fit
            ...
            [Msol,fitOK] = VoutLinearFit_vH1(datatemp);
            m = Msol(1);
            b = Msol(2);
...
yshortmin = min([Vout_data_A;Vout_shifted_A;Vout_data_B(ishort);Vout_shifted_B(ishort)]);
yshortmax = max([Vout_data_A;Vout_shifted_A;Vout_data_B(ishort);Vout_shifted_B(ishort)]);
